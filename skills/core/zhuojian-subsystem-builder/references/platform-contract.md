...
- `presentation.moduleNavigationTheme`：Skill 发布必检的应用级模块导航主题，四个受控颜色须与业务页面配色一致，不接受任意 CSS；SaaS 仅用于当前应用的模块导航、标题身份区及侧栏选中项，不影响其他系统或 iframe 内容。完整约束见 [模块导航与安全迁移](module-navigation-migration.md)。SaaS 对已登记旧版本保留缺失时的默认色回退，不代表新发布可省略。
