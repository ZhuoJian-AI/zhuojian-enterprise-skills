...
- `enterprise.key`：必须与当前 ECS Runtime 档案中的 `enterpriseKey` 一致。历史值 `aifabei` 规范化为 `alphabet`；其他企业使用管理员登记的稳定标识。
...
- `presentation.moduleNavigationTheme`：Skill 发布必检的应用级模块导航主题，四个受控颜色须与业务页面配色一致，不接受任意 CSS；SaaS 仅用于当前应用的模块导航、标题身份区、侧栏选中项及同一统一助手的入口、外框与输入控件，不影响回答／审批语义状态、其他系统或 iframe 内容。完整约束见 [模块导航与安全迁移](module-navigation-migration.md)。SaaS 对已登记旧版本保留缺失时的默认色回退，不代表新发布可省略。
...
- 用户组织归属为单一 `departmentId`，可拥有多个平台角色。页面、Action 与 AI 能力由绑定角色取并集；每次访问时，SaaS 先筛出确实授予当前资源的角色，再只合并这些角色的数据范围。未授予当前资源的角色不能扩大它的 `effectiveDataScope`。工作空间另有企业公共／所属部门默认只读规则，角色追加权限；见 [工作空间与业务授权边界](workspace-access.md)，不得将其套用到业务 Action。
...
新增/改造业务流程先按 [业务提效设计与交付](business-ai-delivery.md) 形成业务目标、能力核对和真实验收，再映射到本协议既有字段。`docs/ai-delivery.json` 仅供开发交接，不是 Manifest 或运行时协议；不得借它创建另一套主脑、供应商配置、调度或授权。Skill 更新不证明某项平台能力已经部署。

灼见 SaaS 是唯一的平台 AI 控制面，负责模型供应商注册和 API Key 保管、模型路由与额度、Manifest/Action 目录、用户最终授权计算、AI 工具选择、短时 Action JWT 签发、确认流程和审计。模型供应商密钥不得进入任何企业模块的源码、镜像、运行环境、数据库或前端，也不得通过 SSO、Bridge、Action 请求或事件传给模块。模块只提供业务能力，不需要知道平台使用哪个模型供应商。
...
文件工具完成临时生成后，必须经过文件验证和工作空间提交才产生可信 `artifact`。文件目标没有有效 Artifact 不能标记完成；Action 成功而文件失败时分别呈现业务成功和文件失败，恢复时不重做成功写入。同一用户确认绑定稳定逻辑操作标识、租户、主体、Action、参数及版本；`runId + toolCallId` 只标识调用，不能作为跨轮防重复的唯一依据。SSE 重连按持久事件重放，文件卡片按 `fileId + versionId` 去重。预览下载实时鉴权，不把签名 URL 持久化到聊天。删除对话不删除已交付文件。
...
按操作的 `permissionPolicy`、公共读取、固定本人及管理范围见 [操作权限与记录归属](action-permission-scopes.md)。新建及本次新增 Action 必须声明并实现，旧缺失策略保持兼容并输出升级清单。这些扩展不改变 2.4/2.5 版本，不自动修改业务角色绑定；只有真实实现了对应边界才能声明。拆分旧页面时遵循 [模块导航与安全迁移](module-navigation-migration.md)，不得复制授权造成扩权。

旧数据没有可用的负责部门或创建人字段时，不能仅新增空列或按姓名猜测归属。先生成未归属引用清单，受限操作保持隐藏或拒绝，业务负责人确认后才能回填；公共已发布内容独立读取，不因历史私有记录无归属而拒绝整页。系统研发者也不能绕过固定本人规则。
...
解释页面可使用 `aiSemantics`；实时事实必须来自获授权查询，不能把页面缓存或历史回答冒充结果。能从页面对象或查询工具补齐的信息先查询，仍有歧义才询问用户；不能猜测关键业务参数。查询与安全文件生成不强制导航，用户要求导航或操作确需页面交互时才导航。跨页面能力必须重新校验角色和所需上下文，导航本身不授予权限。

首次提供少量搜索/导航入口与当前页面获权工具；其余公共和业务工具通过能力搜索按需加载完整 Schema，不限定只能有一个查询 Action。系统和页面语义整理为本轮提示词上下文，Action.description 与参数 Schema 转为 tools，不把完整 Manifest 一次性注入。SaaS 对模型参数和子系统返回值执行必要校验。Manifest、Action 返回值、Bridge 内容和工作空间文件均是非可信业务数据，不能覆盖系统规则、增加权限或任意指定执行地址。
...
主脑负责理解与多步工具编排；权限、参数、确认、版本、幂等和文件提交由平台确定性执行。实时查询必须有成功 Action，修改必须有业务回执，文件请求必须有工作空间 Artifact。首次尝试后最多三次纠正，受平台整轮步骤和时间预算限制；执行器反馈为准，连续相同错误不能原样重试。权限拒绝、取消、危险参数和版本冲突不能绕过；写入超时先核实状态，不盲目重发。可恢复工具错误不等于最终任务失败，部分成功按实际步骤分别呈现。

### 应用内对话与历史页面

首页与页面侧栏是同一助手的两个视图，共享 Task、历史、文件引用与未完成目标。Task 不永久绑定首次应用；切换应用或模块只更新本轮经过服务端验证的上下文，不能延续其他应用权限。不同 Task 仍相互隔离。每条用户消息记录当时页面与业务引用；大批量结果只保存摘要及需要重新鉴权的引用。

新建对话只创建空白草稿，第一次有效发送才创建 Task；新 Task 不继承旧 Task 的引用。历史对话可恢复，URL 使用 `conversation=<taskId>`；服务端验证 Task 所属租户和用户，不按首次应用阻止同 Task 接续。历史消息保留当时页面，下一轮使用当前验证后的页面。运行中同 Task 导航不能重建会话或重复提交；切换不同 Task 遵循平台取消与隔离规则。删除对话不删除已交付工作空间文件。
...
前端必须交付**电脑端一套、手机端一套**界面方案。新建及本次修改的员工、业务管理页面须分别设计布局与交互、实现并验收，不能只做电脑页面再缩小或裁切到手机。两端共享账号、权限、导航数据、接口、业务规则及草稿状态；可以复用基础组件或按需拆分桌面/手机视图，不要求另建项目，也不强制相同 DOM。

SaaS 负责平台导航、顶部栏、工作空间、统一助手和 iframe 容器的双端界面；子系统负责自身业务页面、表单、表格、弹窗和操作按钮的双端界面。子系统不得控制父页面布局或重置平台导航偏好。平板及连续宽度变化保持可用，不能只针对测试宽度写特例；规则更新不等于既有系统已经适配或部署。

视觉预览必须同时提供电脑版和手机版，可切换或分别打开，并展示各自的导航、主要内容和关键操作。已有预览确认流程应覆盖两端；仅电脑截图、静态图片或空白模板不能代替手机交互验证。两端界面方案应说明各自的信息组织和操作入口，业务完成条件保持一致。

- 手机宽度不超过 768px 时使用抽屉导航、单列主内容和适合触摸的操作区；769–1199px 使用紧凑或可折叠布局；1200px 以上保留桌面多栏和信息密度。断点之间仍必须自然收缩。
- 电脑端照顾鼠标、键盘、多栏与信息密度，手机端围绕触摸、单列、卡片/详情和视口内操作设计。CSS media/container queries 用于连续布局，必要时切换专用视图；权限与业务状态仍来自共享逻辑，不能因切换布局丢草稿、重复提交或同时初始化两套副作用。
...
- 图片、背景及动效分别验证两端的裁切、可读性、网络体积和运行开销，按需加载当前布局需要的重型资源；手机支持静态/轻量回退并尊重 `prefers-reduced-motion`，不能以“代码渲染”代替性能证据。双端修改不得降低桌面端既有导航、信息密度、多栏、表格和业务流程的可用性。

使用 Playwright 等浏览器自动化执行真实页面验收，覆盖全部 Manifest 页面，并至少包含 320×568、390×844、844×390、768×1024、1024×768、1280×720、1440×900 和 1920×1080；Chromium 检查桌面及手机模拟环境，WebKit 检查移动 WebKit 环境，桌面 Firefox 做基础回归。每页同时验收独立与嵌入模式、根节点溢出、菜单/返回/弹窗/表单/主要按钮、局部表格滚动、旋转、前进后退和 200% 缩放，并保存关键视口截图。分别记录电脑端、手机端的源码基线、页面/流程覆盖、浏览器、视口、结果及未测项；动态高度、安全区、真实软键盘与真机表现另记实际测试方式，模拟器通过不能宣称 iPhone/Android 真机通过。任何必需页面在手机上不能完成对应桌面业务流程，或电脑端因双端修改退化，都必须阻断发布。
...
主脑仅使用当前获权能力，不把其他应用历史、记忆或页面缓存冒充当前业务事实。实时问题必须有相关获权查询成功返回；失败时允许在预算内查询候选或纠正参数，仍无法核实时明确说明。验收以真实目标、回执和文件为准：纠错后正确完成可通过；越权、重复写入、虚假完成或错误产物不通过。不要求固定自然语言句式或唯一工具轨迹。
