...
旧独立仓库最后发布 `aifabei-subsystem-builder 1.1.0`，把已有 `1.x` 副本接到公共仓库。公共仓库随后提供同名 `1.2.0` 兼容入口，自动安装正式的 `zhuojian-subsystem-builder`。完成迁移后，总 Skill 和所有交接 Skill 都只读取 `ZhuoJian-AI/zhuojian-enterprise-skills` 的 GitHub Release 稳定目录，不读取 `main`，也不要求负责人拥有 GitHub 账号。
...
没有 GitHub 账号也可以下载公开 Release；网络必须能访问 GitHub。旧副本没有更新器时，需要按仓库 README 手工执行一次引导安装。正式总 Skill 在 `1.x` 内自动更新；`2.x` 等跨主版本由管理员明确升级。
