...
SKILL_NAME = "zhuojian-subsystem-builder"
LATEST_MANIFEST_URL = (
    "https://github.com/ZhuoJian-AI/zhuojian-enterprise-skills/"
    "releases/latest/download/zhuojian-subsystem-builder-update-manifest.json"
)
...
def read_url(
    ...
    *,
    ...
    request = urllib.request.Request(
        url,
        headers={"Accept": "application/octet-stream", "User-Agent": "ZhuoJian-Skill-Updater/1"},
    )
    try:
...
def read_local_version(skill_dir: Path, *, bootstrap: bool) -> str | None:
    ...
    if not re.search(r"(?m)^name:\s*[\"']?zhuojian-subsystem-builder[\"']?\s*$", entry_text):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="检查并安装 zhuojian-subsystem-builder 稳定版")
