...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "installed":
            ...
            requested = validate_skill_name(args.skill_name)
            release = releases.get(requested)
            if release is None or release.kind != "core":
                raise ManagedUpdateError(f"企业 Skill 目录没有总 Skill：{requested}")
            selected = [release]
        if not selected:
