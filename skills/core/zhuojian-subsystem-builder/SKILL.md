...
description: "让 AI 用业务需求和服务器登录信息，在已接入灼见的企业 ECS 上新建、修改并部署业务子系统，自动处理统一登录、角色授权、业务操作、文件存储和公司专属交接规则。用户提到灼见 SaaS、企业子系统、ECS、跨部门模块、OSS 迁移或 iframe 接入时使用。"
---
...
涉及角色、公共内容或个人记录时，读取 [操作权限与记录归属](references/action-permission-scopes.md)。可打开页面、可执行操作、可处理的记录分别校验；不能以角色不是全企业范围为由拒绝整页。可选 `permissionPolicy` 只在平台支持且子系统实际实现相应边界后声明，旧契约缺少该字段不自动放宽。
