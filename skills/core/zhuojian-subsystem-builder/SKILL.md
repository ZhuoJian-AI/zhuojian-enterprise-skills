...
涉及模块拆分、导航或既有系统升级时，读取 [模块导航与安全迁移](references/module-navigation-migration.md)，按服务器当前契约生成新旧映射和权限影响。SaaS 提供系统与横向模块导航；嵌入页面隐藏重复导航，保留紧凑业务内容。Skill 自动更新只更新开发规范，不代表业务系统已经适配、部署或同步。
