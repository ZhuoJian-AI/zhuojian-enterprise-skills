...
- 前端必须交付**电脑端一套、手机端一套**界面方案，覆盖本次新建或修改的员工及业务管理页面，分别设计、实现、预览和验收；不能把电脑页缩小或隐藏关键功能作为手机交付。两端共享数据、权限、接口、业务逻辑和草稿状态，可复用组件或按需拆分视图，不强制同一 DOM 或两个独立项目；平板和中间宽度保持可用。具体责任、预览与验收见 [电脑端与手机端双端交付](references/platform-contract.md#电脑平板和手机全端界面)。
