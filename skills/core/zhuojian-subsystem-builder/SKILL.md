...
涉及模块拆分、导航、模块导航主题或既有系统升级时，读取 [模块导航与安全迁移](references/module-navigation-migration.md)，按服务器当前契约生成新旧映射和权限影响。SaaS 提供系统导航、桌面横向模块导航及手机自动换行模块网格；手机不能要求左右滑动才能看全八个模块。嵌入页面隐藏重复导航，保留紧凑业务内容；SaaS 以当前系统主题衔接其模块栏、标题身份区和侧栏选中项，并允许平台导航轨、通用操作及同一统一助手的入口、外框与输入控件使用受控颜色，平台骨架、回答内容和审批语义状态仍保持中性，不把某个系统的风格套给其他系统。每个子系统发布前必须登记与自身页面配色一致、通过对比度校验的 `presentation.moduleNavigationTheme`；缺失或无效时 Skill 的源码、端点及登记前校验必须失败，不得报告“已同步”。只登记受控颜色，不开放 CSS；已上线旧版本仍由 SaaS 安全回退默认色，不因 Skill 更新停用。Skill 自动更新只更新开发规范，不代表业务系统已经适配、部署或同步。
...
涉及让助手理解业务办理顺序、完成条件或主动提示员工时，读取 [业务流程知识、当前页与跨页检查](references/assistant-workflow-guidance.md)。开发 AI 从真实代码与业务话语提取可选 `workflowGuides`；有真实只读检查才登记 `proactiveCheck` 或独立的 `globalCheck`。前者仍仅在员工开启且当前页可见时查询；后者仅供已登录 SaaS 外壳在获权范围内跨页低频检查，不依赖当前页面、离线运行或自动启动助手。两者保持 2.4/2.5，不能把本地交付资料变成运行脚本；建议仍由员工选择进入同一助手，写入须重新授权与确认。
