...
涉及模块拆分、导航、模块导航主题或既有系统升级时，读取 [模块导航与安全迁移](references/module-navigation-migration.md)，按服务器当前契约生成新旧映射和权限影响。SaaS 提供系统导航、桌面横向模块导航及手机自动换行模块网格；手机不能要求左右滑动才能看全八个模块。嵌入页面隐藏重复导航，保留紧凑业务内容；SaaS 以当前系统主题衔接其模块栏、标题身份区和侧栏选中项，并允许平台导航轨、通用操作与统一助手入口使用受控的边框、焦点、图标和轻量晕染，平台骨架仍保持中性，不把某个系统的风格套给其他系统或助手内容。每个子系统发布前必须登记与自身页面配色一致、通过对比度校验的 `presentation.moduleNavigationTheme`；缺失或无效时 Skill 的源码、端点及登记前校验必须失败，不得报告“已同步”。只登记受控颜色，不开放 CSS；已上线旧版本仍由 SaaS 安全回退默认色，不因 Skill 更新停用。Skill 自动更新只更新开发规范，不代表业务系统已经适配、部署或同步。
