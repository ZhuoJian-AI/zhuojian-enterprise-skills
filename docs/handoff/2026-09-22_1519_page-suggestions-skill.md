...
- Skill 实现子任务：可选协议文档、模板适配器、Runtime 非敏感发现白名单、行为测试；主任务集成时同步根 TASKS/AGENTS。版本、发布包和稳定安装未改。
...
- 主任务实际跨仓测试通过：本模板消息交给 SaaS 真正解析器；SaaS 后端 capability 返回值交给本 Runtime sanitizer。不是两边各测各的手写示例。覆盖等待中空快照撤回及旧回执不误结算新请求。

这些结果不能代替真实两服务器、有效员工角色、业务检查结果、真实模型、确认写入及手机真机验收。SaaS 与模板的跨仓实际适配器测试及浏览器集成证据由主任务另外记录。

独立复核未发现剩余阻断，并另行运行两份实际适配器 Node 测试通过。前向演练以“订单延期/缺附件提醒、员工下班后也希望自动处理”为需求，评审代理读取实际 Skill 后正确选择在原订单页接入真实检查、来源与证据版本，复用唯一助手；将 `backgroundDelegation/unattended=false` 的离线自主处理明确列为平台未支持，而非子系统 cron、模型直连或模拟员工。没有要求小白负责人取得 SaaS root，没有把本地测试当部署。演练只读，未产生业务或外部副作用。
